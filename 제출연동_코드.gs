 ...
 *   · 추론게임          ← Ⅰ-06 활동 ⑪
 *   ── Ⅱ단원 수업 활동 ──
 *   · Ⅱ 학습활동기록    ← unit2.html 활동 기록지 (전처리 체험 · 유형 고르기 ·
 *                        k-최근접 이웃 · 선형 회귀 · 퍼셉트론 · O/X 를 한 줄로)
...
var SHEETS = {
  ...
  /* ── Ⅱ단원 ─────────────────────────────────────────────
     unit2.html 의 «활동 기록지» 한 장. 학생이 그 쪽에서 눌러 본 결과가
     자동으로 모여 한 줄로 들어옵니다.                                */
  u2learn: {
    name: 'Ⅱ 학습활동기록',
    head: ['제출 시각', '분반', '이름',
           '전처리 체험', '유형 고르기', 'k-최근접 이웃', '선형 회귀', '퍼셉트론', 'O/X',
           '① 전처리를 안 했다면', '② k를 바꾸니 답이 달라진 까닭',
           '③ 우리 학교에서 풀어 보고 싶은 문제',
           '자기평가 1', '자기평가 2'],
    width: [140, 70, 110, 110, 110, 230, 110, 200, 150, 480, 480, 540, 80, 80],
    row: function (d) {
      return [new Date(), d.cls || '', d.name || '',
              d.prep || '', d.kinds || '', d.knn || '', d.reg || '', d.percep || '', d.ox || '',
              d.q1 || '', d.q2 || '', d.q3 || '',
              d.rub1 || '', d.rub2 || ''];
    }
  },
